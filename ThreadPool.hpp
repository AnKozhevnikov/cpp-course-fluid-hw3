...
class ThreadPool
{
  private:
    ...
    template <class T> class Future : public BaseFuture
    {
      private:
        std::future<T> future;

      public:
        Future(uint32_t nid, std::future<T> &&future) : BaseFuture(nid), future(std::move(future))
        {
        }
        void wait()
        {
    ...
    template <class F, class... Args> class PackagedTask : public BasePackagedTask
    {
      private:
      ...
      public:
        PackagedTask(uint32_t nid, F &&f, Args &&...nargs)
            : BasePackagedTask(nid), func(std::forward<F>(f)), args(std::make_tuple(std::forward<Args>(nargs)...)),
  ...
  public:
    void workerThread(std::stop_token stoken)
    {
        while (true)
        {
            std::unique_ptr<BasePackagedTask> task;
            {
                ...
                condition.wait(lock, [this, &stoken] { return stoken.stop_requested() || !tasks.empty(); });
    ...
    void init(int numThreads)
    {
        ...
        {
            workers.emplace_back([this](std::stop_token stoken) { this->workerThread(stoken); });
        }
    }
    ...
    template <class F, class... Args> int enqueue(F &&f, Args &&...args)
    {
        uint32_t id = nextId++;
        auto task = std::make_unique<PackagedTask<F, Args...>>(id, std::forward<F>(f), std::forward<Args>(args)...);
        {
            std::lock_guard<std::mutex> lock(queueMutex);
    ...
    {
        std::unique_lock<std::mutex> lock(queueMutex);
        futureCondition.wait(lock, [this, id] { return futures.find(id) != futures.end(); });
        futures[id]->wait();
    }
